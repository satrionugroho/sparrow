defmodule Sparrow.FCM.V1.Pool.Subscription do
  use Supervisor

  @default_endpoint "iid.googleapis.com"
  ...
  def init(config) do
    pool_conf =
      Enum.map(config, fn c ->
        {c[:path_to_json], get_fcm_pool_config(c)}
      end)

    for {path_to_json, {conf, _tags}} <- pool_conf do
    ...
    children =
      for {{_json, {conf, tags}}, index} <- Enum.with_index(pool_conf) do
        id = String.to_atom("Sparrow.Fcm.Pool.ID.#{index}")

        %{
          id: id,
          start: {Sparrow.H2Worker.Pool, :start_link, [conf, :fcm_subscription, tags]}
        }
      end
  ...
  @spec get_fcm_pool_config(Keyword.t()) :: {Sparrow.H2Worker.Pool.Config.t(), [atom()]}
  defp get_fcm_pool_config(raw_pool_config) do
    uri = Keyword.get(raw_pool_config, :endpoint, @default_endpoint)
